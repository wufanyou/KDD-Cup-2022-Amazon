...
def make_log_bucket_position(relative_pos, bucket_size, max_position):
    max_position = torch.tensor(max_position,dtype=torch.long)
    sign = torch.sign(relative_pos)
    mid = bucket_size // 2
    abs_pos = torch.where((relative_pos < mid) & (relative_pos > -mid), mid - 1, torch.abs(relative_pos))
    log_pos = torch.ceil(torch.log(abs_pos / mid) / torch.log((max_position - 1) / mid) * (mid - 1)) + mid
    log_pos = log_pos.type(torch.int32)
    bucket_pos = torch.where(abs_pos <= mid, relative_pos, log_pos * sign).type(torch.int32)
    return bucket_pos
